...
describe('Testing routes', () => {
    ...
    describe('GET /messages', () => {
        const FOO_USERNAME = 'foo',
            FOO_MESSAGE_TEXT = 'hello';
        ...
        beforeEach(done => {
            ...
            function findAuthor () {
                User.findOne({ username: FOO_USERNAME }, (err, user) => {
                    if (err)
                        console.error(err);
                    else
                        saveMessage(user);
        ...
        it('should response with message', done => {
            chai.request(app)
                ...
                .end((err, res) => {
                    should.not.exist(err);
                    res.body[0].should.have.property('id');
                    res.body[0].should.have.property('msg', FOO_MESSAGE_TEXT);
                    res.body[0].should.have.property('user');
                    res.body[0].user.should.have.property('id');
                    res.body[0].user.should.have.property('username', FOO_USERNAME);
                    done();
                });
        });
    });
